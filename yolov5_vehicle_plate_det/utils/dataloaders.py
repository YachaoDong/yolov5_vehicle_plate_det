...
def ji_LoadImages(img0, img_size, stride, auto):
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                ...
                # plate obj, poly,   cls_id, color_id, x1, y1, x2, y2, x3, y3, x4, y4 (10)
                if any(len(x) > 7 for x in lb):  # is segment
                    classes = np.array([x[0] for x in lb], dtype=np.float32)
                    obj_colors = np.array([x[1] for x in lb], dtype=np.float32)
                    segments = [np.array(x[2:], dtype=np.float32).reshape(-1, 2) for x in lb]  # (cls, xy1...)
                    lb = np.concatenate((classes.reshape(-1, 1), obj_colors.reshape(-1, 1), segments2boxes(segments)), 1)  # (cls, xywh)
...
def dataset_stats(path='coco128.yaml', autodownload=False, verbose=False, profile=False, hub=False):
    ...
    with open(check_yaml(yaml_path), errors='ignore', encoding='utf8') as f:
