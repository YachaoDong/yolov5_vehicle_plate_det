...
def output_to_target(output):
    ...
    for i, o in enumerate(output):
        for *box, conf, cls, color in o.cpu().numpy():

            targets.append([i, cls, color, *list(*xyxy2xywh(np.array(box)[None])), conf])
    return np.array(targets)


@threaded
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, vehicle_color=None, max_size=1920, max_subplots=16):
    ...
    for i in range(i + 1):
        ...
        if len(targets) > 0:
            ...
            boxes = xywh2xyxy(ti[:, 3:7]).T
            classes = ti[:, 1].astype('int')
            obj_clrs = ti[:, 2].astype('int')
            ...
            for j, box in enumerate(boxes.T.tolist()):
                ...
                # obj color
                obj_clr = obj_clrs[j]
                obj_clr_color = colors(obj_clr)
                obj_clr = vehicle_color[obj_clr] if vehicle_color else obj_clr


                if labels or conf[j] > 0.25:  # 0.25 conf thresh
                    label = f'{cls} {obj_clr}' if labels else f'{cls} {obj_clr} {conf[j]:.1f}'
...
def plot_labels(labels, names=(), save_dir=Path('')):
    ...
    c, b = labels[:, 0], labels[:, 2:].transpose()  # classes, boxes
    ...
    # rectangles
    labels[:, 2:4] = 0.5  # center
    labels[:, 2:] = xywh2xyxy(labels[:, 2:]) * 2000
    img = Image.fromarray(np.ones((2000, 2000, 3), dtype=np.uint8) * 255)
    for cls, obj_color, *box in labels[:1000]:
