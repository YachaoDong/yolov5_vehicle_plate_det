...
def crop_img_and_count_plate(all_ocr_data, ocr_data_path):
    '''
    ...
    with open(join(ocr_txt_dir, 'all_ocr.txt'), 'w', encoding='utf-8') as f:
        ...
        for cur_ocr_data in all_ocr_data:
            ...
            if plate_content == 'NONE':
                none_count.append([w, h, wh])
            else:
                wavy_count = plate_content.count('~')
                underline_count = plate_content.count('_')
                all_count.append([w, h, wh, wavy_count, underline_count])
                # save crop img xyxy
                ...
                f.write(crop_img_path + '\t' + plate_content + '\n')
...
if __name__ == '__main__':
    ...
    txt_dir_path = join(opt.ocr_data_dir, 'ocr_labels')
    ...
    # 统计数据
    # /data,
    crop_img_and_count_plate(all_ocr_data, opt.ocr_data_dir)
