...
def non_max_suppression(prediction,
    ...
    """
    # prediction: [batch, num_anchors(3个yolo预测层), (x+y+w+h+1+num_classes + num_colors)] = [1, 18900, 25]  3个anchor的预测结果总和
    ...
    t = time.time()
    # [num_obj, x1y1x2y2+object_conf+cls+color] = [5, 7]
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if labels and len(labels[xi]):
            print('using autolabing, pls process color')
            lb = labels[xi]
            # v: xywh + conf + num_cls + num_color
            v = torch.zeros((len(lb), nc + 5 + nco), device=x.device)
            v[:, :4] = lb[:, 2:6]  # box
            v[:, 4] = 1.0  # conf
            v[range(len(lb)), lb[:, 0].long() + 5] = 1.0  # cls
            v[range(len(lb)), lb[:, 1].long() + 5 + nc] = 1.0  # color
            x = torch.cat((x, v), 0)
        ...
        if multi_label:
            ...
            conf, j = x[:, 5:nc+5].max(1, keepdim=True)  # best class
...
def print_mutation(results, hyp, save_dir, bucket, prefix=colorstr('evolve: ')):
    ...
    keys = ('metrics/precision', 'metrics/recall', 'metrics/mAP_0.5', 'metrics/mAP_0.5:0.95', 'val/box_loss',
            'val/obj_loss', 'val/cls_loss', 'val/color_loss') + tuple(hyp.keys())  # [results + hyps]
