...
config.train_options = {
    # for train
    'resume_from': r'D:\CodeFiles\data\vehicle_data\ocr_runs\ch_rec_moblie_crnn_mbv3_pretrained.pth',  # 继续训练地址
    'third_party_name': '',  # 加载paddle模型可选
    'checkpoint_save_dir': rf"D:\CodeFiles\data\vehicle_data\ocr_runs\output\{config.exp_name}\checkpoint",  # 模型保存地址，log文件也保存在这里
    'device': 'cuda:0',  # 不建议修改
    'epochs': 300,
...
config.dataset = {
    'alphabet': r'../torchocr/datasets/alphabets/plate.txt',
    'train': {
        'dataset': {
            'type': 'RecTextLineDataset',
            'file': r'D:\CodeFiles\data\vehicle_data\ocr_runs\train.txt',
            'input_h': 32,
            'mean': 0.5,
            'std': 0.5,
            'augmentation': False,
    ...
    'eval': {
        'dataset': {
            'type': 'RecTextLineDataset',
            'file': r'D:\CodeFiles\data\vehicle_data\ocr_runs\test.txt',
            'input_h': 32,
            'mean': 0.5,
            'std': 0.5,
            'augmentation': False,
